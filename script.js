document.addEventListener('DOMContentLoaded', function () {
    ...
    var defaultThesisData = {
        ...
        stages: [
            { title: 'Fase 1: Elección y Delimitación del Tema', deadline: null, tasks: [ { description: 'Realizar lluvia de ideas sobre posibles temas.', completed: false }, { description: 'Investigar antecedentes de los temas seleccionados.', completed: false }, { description: 'Definir el tema final y delimitar su alcance.', completed: false } ] },
            { title: 'Fase 2: Planteamiento del Problema', deadline: null, tasks: [ { description: 'Redactar la descripción del problema.', completed: false }, { description: 'Formular la pregunta de investigación principal.', completed: false }, { description: 'Formular preguntas de investigación secundarias.', completed: false }, { description: 'Escribir la justificación e importancia del estudio.', completed: false } ] },
            { title: 'Fase 3: Construcción del Marco Teórico', deadline: null, tasks: [ { description: 'Identificar las bases teóricas clave.', completed: false }, { description: 'Buscar y recopilar literatura relevante (artículos, libros).', completed: false }, { description: 'Redactar los antecedentes de la investigación.', completed: false }, { description: 'Desarrollar los conceptos y teorías centrales.', completed: false } ] },
    ...
    function loadData() {
        try {
            ...
            if (!data.projectInfo) {
                data.projectInfo = clone(defaultThesisData.projectInfo);
            }
            return data;
        } catch (e) {
    ...
    /* ====== Migración (opcional pero bueno tenerla) ====== */
    (function ensureStages() {
        if (!thesisData.stages) thesisData.stages = [];
        function have(t) {
            return thesisData.stages.some(function (s) { return s.title === t; });
        }
        if (!have('Fase 5: Resultados y Discusión') && defaultThesisData.stages[4]) {
            thesisData.stages.push(clone(defaultThesisData.stages[4]));
        }
        if (!have('Fase 6: Conclusiones y Recomendaciones') && defaultThesisData.stages[5]) {
    ...
    var importFileInput = document.getElementById('import-json');

    /* ====== Lógica de Información del Proyecto ====== */
    ...
    function renderRoadmap() {
        ...
        (thesisData.stages || []).forEach(function (stage, si) {
            ...
            dl.textContent = stage.deadline ? (isOverdue ? '¡Atrasado! Límite: ' + stage.deadline : 'Fecha Límite: ' + stage.deadline) : 'Sin fecha límite';
            titleBox.appendChild(h2);
            titleBox.appendChild(dl);
            ...
            (stage.tasks || []).forEach(function (task, ti) {
                ...
                if (!isLocked) {
                    ...
                    editB.className = 'task-action-btn';
                    editB.title = 'Editar tarea';
                    editB.textContent = '✏️';
                    editB.addEventListener('click', function () { editTask(si, ti); });
                    var delB = document.createElement('button');
                    delB.className = 'task-action-btn';
                    delB.title = 'Eliminar tarea';
                    delB.textContent = '🗑️';
            ...
            if (!isLocked) {
                ...
                ninp.type = 'text';
                ninp.placeholder = 'Añadir nueva tarea...';
                var nbtn = document.createElement('button');
                nbtn.type = 'button';
                nbtn.textContent = 'Añadir';
    ...
    function renderGanttChart() {
        ...
        (thesisData.stages || []).forEach(function (stage) {
            ...
            var titleParts = stage.title.split(':');
            label.textContent = (titleParts[1] ? titleParts[1] : stage.title).trim();
            var row = document.createElement('div');
    ...
    if (exportBtn) {
        exportBtn.addEventListener('click', function () {
            var payload = {
                ...
                student: thesisData.projectInfo,
                data: thesisData
            };
        ...
        importFileInput.addEventListener('change', function (e) {
            ...
            reader.onload = function (event) {
                try {
                    ...
                    } else if (obj && obj.stages) { // Para compatibilidad con versiones antiguas
                        thesisData = obj;
                        saveData();
                        renderAll();
                        alert('¡Progreso importado con éxito!');
                    }
                    if (obj && obj.student) { // Para compatibilidad
                        thesisData.projectInfo = obj.student;
                        saveData();
                        renderAll();
                    }
                } catch (error) {
